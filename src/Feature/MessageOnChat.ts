...
export class MessageOnVoiceChat implements BotFeature {
    ...
    async getTextChannel(client: Client, channelId = process.env.BOT_CHANNEL): Promise<TextChannel | null> {
        if (!channelId) return null;

        return client.channels.fetch(channelId) as Promise<TextChannel>
    }

    getVoiceTextUpdate(before: VoiceState, after: VoiceState, me: string): string | null {
        if (before.member?.id === me) return null;

        const afterNames = after.channel?.members.map(member => member.displayName).join(', ');
        const beforeNames = before.channel?.members.map(member => member.displayName).join(', ');

        if (!before.channel && after.channel) {
            return `${after.member?.displayName} has entered the ${after.channel.name} chat. Party time with ${afterNames}`
        }
        if (before.channel && !after.channel) {
            return `${before.member?.displayName} has left the ${before.channel.name} chat. ${beforeNames ?? 'nobody'} remain`
        }
